...
export async function mongoWatchProcessorTrigger(cs: StateConnection, watchCollection: string, processor: Processor, filter: any) {
    ...
    return cs.db.collection(watchCollection).watch(
        [
            { $match: { $and: [{ 'operationType': { $in: ['insert', 'update'] } }, { 'fullDocument.partition_key': cs.tenent.email }].concat(filter ? Object.keys(filter).reduce((acc, i) => { return { ...acc, ...{ [`fullDocument.${i}`]: filter[i] } } }, {}) as any : []) } }
...
export async function mongoCollectionDependency(cs: StateConnection, stateManager: StateManagerInterface, stateSlice: string, collection: string, actiontype: string) {
    ...
    var inventoryStreamWatcher = db.collection(collection).watch([
        { $match: { $and: [{ 'operationType': { $in: ['insert'] } }, { 'fullDocument.partition_key': tenent.email }].concat(sequence ? { 'fullDocument.sequence': { $gt: sequence } } as any : []) } },
...
async function readCollectionfromSequence({ db, tenent }: StateConnection, collection: string, sequence: number, filter: any, callback: (doc: any, continuation: any) => Promise<void>): Promise<void> {
    ...
    const cursor = await db.collection(collection).aggregate(
        [
            { $match: { $and: [{ 'partition_key': tenent.email }].concat(filter || []).concat(sequence ? { 'sequence': { $gt: sequence } } as any : []) } },
