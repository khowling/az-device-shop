...
const getCategories = cache(async () => {
    ...
    return await db.collection('products').find({ type: 'Category' })
})
...
const imageUrl = (image) => {
    if (image) {
        if (image.url) {
            ...
            return `/api/file/${image.pathname}`
        else {
            return 'http://placehold.it/50x50'
        }
    } else  return 'http://placehold.it/50x50'

}

export default async function Page() {

   const cats = await(await getCategories()).toArray()

    return (
      <>
       
      <div className="container mx-auto px-6">
      { cats.filter(c => c.position === 'hero').map(c => 
        
        <div className="h-64 rounded-md overflow-hidden bg-cover bg-center" style={{backgroundImage: `url('${imageUrl(c.image)}')`}}>
            <div className="bg-gray-900 bg-opacity-50 flex items-center h-full">
                <div className="px-10 max-w-xl">
                    <h2 className="text-2xl text-white font-semibold">{c.heading}</h2>
                    <p className="mt-2 text-gray-400">{c.description}</p>
