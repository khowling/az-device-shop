...
const storageConnectionString = `DefaultEndpointsProtocol=https;AccountName=${process.env.STORAGE_ACCOUNT};AccountKey=${process.env.STORAGE_MASTER_KEY};EndpointSuffix=core.windows.net`;
    ...
    processEvents: async (events, context) => {
      if (events.length > 0) {
        ...
        for (const event of events) {
          console.log(`Received partition: ${context.partitionId}: ${JSON.stringify(event.body)}`)
        }
        ...
        try {
          ...
          //console.log(`Checkpointed partition: ${context.partitionId}: event with sequenceNumber: ${events[events.length - 1].sequenceNumber}`)
